...
export class Settings {
  ...
  public PaymentMethodGetOption = async (
    request: any,
    response: Hapi.ResponseToolkit
  ): Promise<any> => {
    logger.info(`ROUTE API CALL => \n ${request.url.href}`);
    try {
      ...
      console.log("decodedToken", decodedToken);
      const entity = await this.resolver.PaymentMethodGetOptionV1(
        request.payload,
        decodedToken
      );
  ...
  public getSettingsData = async (
    ...
    try {
      ...
      console.log("decodedToken", decodedToken);
      const entity = await this.resolver.getSettingsDataV1(
        request.payload,
        decodedToken
      );
